...
#    image = cv2.GaussianBlur(image, ksize=(9,9), sigmaX=0, sigmaY=0)
    ...
    if circles is not None:
        ...
        for i, circle in enumerate(circles[0, :]):
            ...
            center_z = circle[1]  # Z座標をスケールに合わせて戻す
            radius = circle[2]    # 半径もスケールに合わせて戻す

            # ワールド座標系に変換
            world_center = obj.matrix_world @ Vector((center_x, target_y, center_z))
            ...
            cv2.circle(image, [center_x, center_z], radius, 255, thickness=2)
...
# スクリプトを実行
extract_circles_near_point_with_hough_transform(tolerance=10, \
